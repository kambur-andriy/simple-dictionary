...
window.showError = function (responseError) {
  $('#error_modal .header').empty();
  $('#error_modal .content').empty();

  if (responseError.response.status === 422) {
    var _responseError$respon = responseError.response.data,
        message = _responseError$respon.message,
        errors = _responseError$respon.errors;
    $('#error_modal .header').text(message);
    $('#error_modal .content').append($('<div />').attr('id', 'errors_list').addClass('list'));

    for (var field in errors) {
      $('#errors_list').append($('<li />').text(errors[field].join('; ')));
    }
  } else {
    var _message = responseError.response.data.message;
    $('#error_modal .header').text(responseError);
    $('#error_modal .content').text(_message);
  }
...
var findWord = function findWord() {
...
var newWord = function newWord(word) {
...
var showWord = function showWord(word) {
  $('#show_word').removeClass('hidden');
  $('#new_word').addClass('hidden');
  $('#word').text(word.text);
  $('#transcription').text("[ ".concat(word.transcription, " ]"));
  ...
  translations.map(function (translation) {
    return showTranslation(translation);
  });
};

var showTranslation = function showTranslation(translation) {
  $('#translations tbody').append($('<tr />').attr('id', translation.id).append($('<td />').html(translation.text)).append($('<td />').html(translation.example)).append($('<td />').addClass('collapsing').append($('<div />').addClass('ui button icon red basic remove-translation').append($('<i />').addClass('trash icon')))));
};
...
var addWord = function addWord() {
  ...
  axios.post('/add-word', newWord).then(function (response) {
...
var addTranslation = function addTranslation() {
  ...
  axios.post('/add-translation', newWord).then(function (response) {
...
var removeTranslation = function removeTranslation(translationId) {
  var translation = {
    translationId: translationId
  };
  axios.post('/remove-translation', translation).then(function (response) {
...
$(document).ready(function () {
  ...
  $('#translations').on('click', '.remove-translation', function (event) {
    event.preventDefault();
    var translationId = $(this).parents('tr').attr('id');
